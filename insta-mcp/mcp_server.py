...
mcp_server = FastMCP(name="Instagram DMs", instructions=INSTRUCTIONS)
...
def list_chats(
    ...
    with optional filters and limits.
    ...
    def thread_summary(thread):
        ...
        return {
            ...
            "last_message": (
                t.get("messages", [{}])[-1] if t.get("messages") else None
            ),
        }
...
def list_messages(thread_id: str, amount: int = 20) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "success": True,
            "messages": [
                m.model_dump() if hasattr(m, "dict") else str(m) for m in messages
            ],
        }
    except Exception as e:
        return {"success": False, "message": str(e)}
...
def list_pending_chats(amount: int = 20) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "success": True,
            "threads": [
                t.model_dump() if hasattr(t, "dict") else str(t) for t in threads
            ],
        }
    except Exception as e:
        return {"success": False, "message": str(e)}
...
def search_threads(query: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "success": True,
            "results": [
                r.model_dump() if hasattr(r, "dict") else str(r) for r in results
            ],
        }
    except Exception as e:
        return {"success": False, "message": str(e)}
...
def get_thread_details(thread_id: str, amount: int = 20) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "success": True,
            "thread": thread.model_dump() if hasattr(thread, "dict") else str(thread),
        }
    except Exception as e:
        return {"success": False, "message": str(e)}
...
if __name__ == "__main__":
    if not instagram_username or not instagram_password:
        print(
            "Error: INSTAGRAM_USERNAME and INSTAGRAM_PASSWORD must be set in the environment."
        )
        exit(1)
