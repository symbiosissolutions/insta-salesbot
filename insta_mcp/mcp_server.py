...
# Load consistent device settings to bypass suspicious activity detection
client.load_settings("/tmp/dump.json")

mcp = FastMCP(name="Instagram DMs", instructions=INSTRUCTIONS)
...
def send_photo_message(username: str, photo_path: str) -> Dict[str, Any]:
    ...
    if not username or not photo_path:
        return {
            "success": False,
            "message": "Username and photo_path must be provided.",
        }
    ...
    try:
        ...
        if result:
            return {
                ...
                "direct_message_id": getattr(result, "id", None),
            }
        else:
...
def send_video_message(username: str, video_path: str) -> Dict[str, Any]:
    ...
    if not username or not video_path:
        return {
            "success": False,
            "message": "Username and video_path must be provided.",
        }
    ...
    try:
        ...
        if full:
            return {
                "success": True,
                "threads": [
                    t.dict() if hasattr(t, "dict") else str(t) for t in threads
                ],
            }
        elif fields:
            return {
                "success": True,
                "threads": [filter_fields(t, fields) for t in threads],
            }
        else:
            ...
            if item_type in [
                ...
                for obj in [clip, media_share, xma, post_share]:
                    if obj:
                        ...
                        shared_url = obj.get("url") or (
                            f"https://www.instagram.com/reel/{shared_code}/"
                            if shared_code
                            else None
                        )
                        shared_info = obj
...
def mark_message_seen(thread_id: str, message_id: str) -> Dict[str, Any]:
    ...
    if not thread_id or not message_id:
        return {
            "success": False,
            "message": "Both thread_id and message_id must be provided.",
        }
...
def list_pending_chats(amount: int = 20) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "success": True,
            "threads": [t.dict() if hasattr(t, "dict") else str(t) for t in threads],
        }
    except Exception as e:
        return {"success": False, "message": str(e)}
...
def get_thread_by_participants(user_ids: List[int]) -> Dict[str, Any]:
    ...
    if not user_ids or not isinstance(user_ids, list):
        return {
            "success": False,
            "message": "user_ids must be a non-empty list of user IDs.",
        }
    try:
        thread = client.direct_thread_by_participants(user_ids)
        return {
            "success": True,
            "thread": thread.dict() if hasattr(thread, "dict") else str(thread),
        }
    except Exception as e:
        return {"success": False, "message": str(e)}
...
def like_media(media_url: str, like: bool = True) -> Dict[str, Any]:
    ...
    try:
        ...
        if result:
            ...
            return {
                "success": False,
                "message": f"Failed to {action.rstrip('d')} post.",
            }
    except Exception as e:
        return {"success": False, "message": str(e)}
    ...
    try:
        ...
        for following_id, followed_user in following.items():
            following_data = {
                ...
                "profile_pic_url": str(followed_user.profile_pic_url)
                if followed_user.profile_pic_url
                else None,
            }
...
def list_media_messages(thread_id: str, limit: int = 100) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"success": False, "message": f"Failed to list media messages: {str(e)}"}


@mcp.tool()
def download_media_from_message(
    message_id: str, thread_id: str, download_path: str = "./downloads"
) -> Dict[str, Any]:
    ...
    try:
        ...
        if not target_message.media:
            return {"success": False, "message": "This message does not contain media"}
        ...
        return {"success": False, "message": f"Failed to download media: {str(e)}"}


@mcp.tool()
def download_shared_post_from_message(
    message_id: str, thread_id: str, download_path: str = "./downloads"
) -> Dict[str, Any]:
    ...
    try:
        ...
        if not shared_url:
            return {
                "success": False,
                "message": "This message does not contain a supported shared post/reel/clip",
            }
        ...
        try:
            ...
            if media.media_type == 1:
                ...
                return {
                    "success": False,
                    "message": f"Unsupported media type: {media.media_type}",
                }
            return {
                "success": True,
            ...
            return {
                "success": False,
                "message": f"Failed to download shared post/reel/clip: {str(e)}",
            }
    except Exception as e:
...
def mute_conversation(thread_id: str, mute: bool = True) -> Dict[str, Any]:
    ...
    try:
        ...
        if result:
            ...
            return {
                "success": False,
                "message": f"Failed to {action.rstrip('d')} conversation.",
            }
    except Exception as e:
        return {"success": False, "message": str(e)}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--username",
        type=str,
        help="Instagram username (can also be set via INSTAGRAM_USERNAME env var)",
    )
    parser.add_argument(
        "--password",
        type=str,
        help="Instagram password (can also be set via INSTAGRAM_PASSWORD env var)",
    )
    ...
    if not username or not password:
        logger.error(
            "Instagram credentials not provided. Please set INSTAGRAM_USERNAME and INSTAGRAM_PASSWORD environment variables in a .env file, or provide --username and --password arguments."
        )
